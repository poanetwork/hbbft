...
Options:
  ...
  --version               \
     Show the version of hbbft.
  -n <n>, --nodes <n>     The total number of nodes [default: 10]
  \
     -f <f>, --faulty <f>    The number of faulty nodes [default: 0]
  -t <txs>, --txs <txs>   \
     The number of transactions to process [default: 1000]
  -b <b>, --batch <b>     The batch \
     size, i.e. txs per epoch [default: 100]
  -l <lag>, --lag <lag>   The network lag between \
     sending and receiving [default: 100]
  --bw <bw>               The bandwidth, in kbit/s \
     [default: 2000]
  --cpu <cpu>             The CPU speed, in percent of this machine's \
     [default: 100]
...
impl<D: DistAlgorithm> Clone for TimestampedMessage<D> where D::Message: Clone
{
    fn clone(&self) -> Self {
        TimestampedMessage { time: self.time,
...
impl<D: DistAlgorithm> TestNode<D> where D::Message: Serialize + DeserializeOwned
{
    /// Creates a new test node with the given broadcast instance.
    fn new(algo: D, hw_quality: HwQuality) -> TestNode<D> {
        let mut node = TestNode { id: algo.our_id().clone(),
                                  algo,
    ...
    fn handle_message(&mut self) {
        ...
        self.algo.handle_message(&ts_msg.sender_id, msg)
    ...
    fn send_output_and_msgs(&mut self) {
        let start = Instant::now();
        let out_msgs: Vec<_> =
            self.algo.message_iter()
                .map(|msg| (msg.target, bincode::serialize(&msg.message).expect("serialize")))
                .collect();
        self.time += start.elapsed() * self.hw_quality.cpu_factor / 100;
        let time = self.time;
        self.outputs.extend(self.algo.output_iter().map(|out| (time, out)));
    ...
    where D::Message: Serialize + DeserializeOwned + Clone
{
        ...
        where F: Fn(NodeUid, BTreeSet<NodeUid>) -> D
    {
        ...
        let new_node_by_id =
            |id: NodeUid| (id, TestNode::new(new_algo(id, node_ids.clone()), hw_quality));
        let mut network =
            TestNetwork { nodes: (0..good_num).map(NodeUid).map(new_node_by_id).collect(), };
        let initial_msgs: Vec<_> = network.nodes
                                          .values_mut()
...
fn parse_args() -> Result<Args, docopt::Error> {
    Docopt::new(USAGE)?.version(Some(VERSION.to_string()))
                       .parse()?
                       .deserialize()
}
...
fn main() {
    ...
    println!("{} transactions, {} bytes each, ≤{} per epoch",
             args.flag_txs, args.flag_tx_size, args.flag_b);
    println!("Network lag: {} ms, bandwidth: {} kbit/s, {:5.2}% CPU speed",
    ...
    let new_honey_badger = |id: NodeUid, all_ids: BTreeSet<NodeUid>| {
        let netinfo = Rc::new(NetworkInfo::new(id,
                                               all_ids,
                                               sk_set.secret_key_share(id.0 as u64),
                                               pk_set.clone()));
        HoneyBadger::builder(netinfo).batch_size(args.flag_b)
                                     .build_with_transactions(txs.clone())
                                     .expect("Instantiate honey_badger")
                                     .0
    };
    let hw_quality = HwQuality { latency: Duration::from_millis(args.flag_lag),
