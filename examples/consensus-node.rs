...
Usage:
  consensus-node --bind-address=<host:port> \
                     [--value=VALUE] [--remote-address=<host:port>]...
  consensus-node (--help | \
                     -h)
...
fn parse_args() -> Args {
    let args = Docopt::new(USAGE).unwrap_or_else(|e| e.exit())
                                 .version(Some(VERSION.to_owned()))
                                 .parse()
                                 .unwrap_or_else(|e| e.exit());
    Args { value: if args.get_count("--value") > 0 {
